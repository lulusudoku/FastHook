...
//F1 02 00 58 ; ldr x17, #92 1f
//1F 00 11 EB ; cmp x0, x17  		;`@X0` stores ArtMethod pointer
//61 02 00 54 ; bne #76		 		;If: target method not equals with current method, continue find
//F1 03 00 AA ; mov x17, x0  		;else: store current method to `@X17`
//A0 02 00 58 ; ldr x0, #84 2f  	;replace current method with hook method

//FF 83 02 D1 ; sub	sp, sp,	#0xa0   ;save X30
//FE 07 02 A9 ; stp x30, x1, [sp, #0x20]
//E2 0F 03 A9 ;	stp x2, x3, [sp, #0x30]
//E4 17 04 A9 ;	stp x4, x5, [sp, #0x40]
//E6 1F 05 A9 ;	stp	x6, x7, [sp, #0x50]
//E0 07 06 6D ; stp d0,	d1, [sp, #0x60]		
//E2 0F 07 6D ; stp d2,	d3, [sp, #0x70]
//E4 17 08 6D ; stp d4,	d5, [sp, #0x80]
//E6 1F 09 6D ; stp d6,	d7, [sp, #0x90]

//E1 03 11 AA ; mov x1, x17
//E2 03 00 91 ; mov x2, sp			;move `@sp` to `@X2`
//71 01 00 58 ; ldr x17, #44 3f		;Jump to hook method
//20 02 3F D6 ; blr x17
//FE 13 40 F9 ; ldr x30, [sp, #0x20]		;restore X30 
//FF 83 02 91 ; add	sp, sp,	#0xa0
//C0 03 5F D6 ; ret
//11 01 00 58 ; ldr x17, #32 4f
//20 02 1F D6 ; br x17
//00 00 00 00 00 00 00 00 ; 1f:art target method
//00 00 00 00 00 00 00 00 ; 2f:hook method
//00 00 00 00 00 00 00 00 ; 3f:hook method entry point
//00 00 00 00 00 00 00 00 ; 4f:other entry point, if x0!= x17, skip hook, execute original method directly
unsigned char quick_hook_trampoline_[] = {
		0xf1, 0x02, 0x00, 0x58,
        0x1f, 0x00, 0x11, 0xeb,
        0x61, 0x02, 0x00, 0x54,
...
//F0 00 00 58 ; ldr x16, #0x1c
//00 02 3F D6 ; blr x16
//FE 13 40 F9 ; ldr x30, [sp, #0x20]		;restore X30 
//FF 83 02 91 ; add	sp, sp,	#0xa0
//C0 03 5F D6 ; ret
//00 00 00 00 00 00 00 00 ; 1f:hook method
//00 00 00 00 00 00 00 00 ; 2f:hook method entrypoint
unsigned char hook_trampoline_[] = {
		0xf0, 0x03, 0x00, 0xaa,
		0x20, 0x02, 0x00, 0x58,
		0xff, 0x83, 0x02, 0xd1,
...
//00 02 1f d6 ; br x16
//00 00 00 00 00 00 00 00 ; 1f:original method
//00 00 00 00 00 00 00 00 ; 2f:original method entry point
unsigned char target_trampoline_[] = {
        0x60, 0x00, 0x00, 0x58,
        0x90, 0x00, 0x00, 0x58,
        0x00, 0x02, 0x1f, 0xd6,
